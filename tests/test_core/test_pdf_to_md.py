...
class PDFToMarkdownTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_pdf_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "test_datas", "test.pdf")
        )
        # 确保测试PDF文件存在
        self.assertTrue(
            os.path.exists(self.test_pdf_path), f"测试PDF文件不存在: {self.test_pdf_path}"
        )

        # 设置测试输出目录
        self.test_output_dir = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), "..", "test_file_parsing", "outputs", "test_pdf_to_md"
            )
        )
        # 确保输出目录存在
    ...
    def test_custom_converter_registration(self):
        ...
        result = converter.convert_pdf_to_markdown(
            pdf_path=self.test_pdf_path, output_dir=self.test_output_dir
        )
    ...
    def test_file_not_found(self):
        ...
        with self.assertRaises(FileNotFoundError):
            convert_pdf_to_markdown(pdf_path=non_existent_path, output_dir=self.test_output_dir)

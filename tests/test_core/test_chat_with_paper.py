...
class TestPDFProcessor(unittest.TestCase):
    ...
    def test_process_pdf(self, mock_convert):
        ...
        # 处理PDF
        result = processor.process_pdf(pdf_path=self.test_pdf_path, output_dir=self.temp_dir)
...
class TestPaperChat(unittest.TestCase):
    ...
    def test_init(self, mock_model_manager_class):
        ...
        # 初始化PaperChat
        chat = PaperChat(
            pdf_processor=mock_pdf_processor, image_text_mode=True, prompt_type="custom_prompt"
        )
    ...
    def test_download_paper_from_arxiv(self, mock_download):
        ...
        mock_download.assert_called_once_with(
            paper_id="1234.5678", output_dir=ANY, get_metadata=True
        )
    ...
    def test_chat_with_paper(self, mock_chat):
        ...
        result = chat.chat_with_paper(
            user_query="测试问题", task_type="test_task", cleanup_after_chat=False
        )
        ...
        # 验证提示词处理
        mock_prompt_config.get_prompt_template.assert_called_once_with(
            chat.prompt_type, "test_task"
        )
    ...
    def test_stream_chat_callback(self, mock_stream_callback):
        ...
        chat.stream_chat_callback(
            user_query="流式测试问题", callback=test_callback, cleanup_after_chat=False
        )

        # 验证回调函数收集的结果
        self.assertEqual(
            collected_responses, ["流式回复第1部分", "流式回复第2部分", "流式回复第3部分"]
        )
        ...
        self.assertEqual(chat.conversation_history[1]["role"], "assistant")
        self.assertEqual(
            chat.conversation_history[1]["content"], "流式回复第1部分流式回复第2部分流式回复第3部分"
        )
...
class TestConvenienceFunctions(unittest.TestCase):
    ...
    def test_download_and_chat_with_paper(self, mock_paper_chat_class):
        ...
        mock_paper_chat.download_paper_from_arxiv.assert_called_once_with(
            paper_id="1234.5678", output_dir=None
        )
        ...
        self.assertEqual(
            convert_kwargs["images_cache_dir"], "/path/to/images_cache"
        )  # 修改为images_cache_dir
...
if __name__ == "__main__":
    ...
    with PaperChat(pdf_processor=text_processor) as paper_chat:
        ...
        # 转换为Markdown
        paper_chat.convert_paper_to_markdown(pdf_path=pdf_path, strip_references=True)

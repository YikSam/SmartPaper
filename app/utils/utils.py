...
def save_uploaded_file(file_content: bytes, file_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"file_id": file_id, "file_path": file_path, "success": True}
    except Exception as e:
        logger.error(f"保存上传文件失败: {e}")
        return {"file_id": file_id, "file_path": "", "success": False, "error": str(e)}
    ...
    def stream_generator(text_iterator: Iterator[str]) -> Iterator[str]:
        ...
        try:
            for chunk in text_iterator:
                if chunk:
                    yield format_sse_message({"content": chunk, "done": False})

            # 发送结束信号
            yield format_sse_message({"content": "", "done": True})
        except Exception as e:
            logger.error(f"流式生成出错: {e}")
            # 发送错误信息
            yield format_sse_message(
                {"content": f"生成过程中出现错误: {str(e)}", "done": True, "error": str(e)}
            )

    @staticmethod
    def stream_generator_with_image_processing(
        text_iterator: Iterator[str], images_cache_dir: str
    ) -> Iterator[str]:
        """
                ...
                if chunk:
                    ...
                    processed_chunk, img_ref_buffer, collecting_img_ref = (
                        find_and_replace_image_in_stream(
                            chunk, img_ref_buffer, collecting_img_ref, images_cache_dir
                        )
                    )

                    # 只有当处理后的块有内容时才发送
                    if processed_chunk:
                        yield format_sse_message({"content": processed_chunk, "done": False})

            # 如果还有未处理完的图片引用，作为普通文本发送
            if img_ref_buffer:
                yield format_sse_message({"content": img_ref_buffer, "done": False})
        ...
        for key, value in llm_config.items():
            ...
            if models_config:
                if isinstance(models_config[0], dict):
                    ...
                    for model_name in models_config:
                        models.append({"name": model_name, "context_length": 32768})  # 默认值
            ...
            if key == default_provider and models:
                if 0 <= default_model_index < len(models):
                    default_model = (
                        models[default_model_index].get("name")
                        if isinstance(models[default_model_index], dict)
                        else models[default_model_index]
                    )
                else:
                    default_model = (
                        models[0].get("name") if isinstance(models[0], dict) else models[0]
                    )
...
def update_config(update_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": success, "error": None if success else "保存配置失败"}
    except Exception as e:
...
def get_prompt_list(prompt_type: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"prompts": prompts, "success": True}
    except Exception as e:
        logger.error(f"获取提示词列表失败: {e}")
        return {"prompts": {}, "success": False, "error": str(e)}
...
def get_prompt_detail(prompt_type: str, prompt_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if template is None:
            return {"success": False, "error": f"未找到提示词: {prompt_type}/{prompt_name}"}

        return {"template": template, "description": description, "success": True}
    except Exception as e:
...
def update_prompt(
    prompt_type: str, prompt_name: str, template: str, description: Optional[str] = None
) -> Dict[str, Any]:
    """
    更新或创建提示词
    ...
    """
    try:
        success = prompt_config.set_prompt(
            prompt_type, prompt_name, template, description, auto_save=True
        )

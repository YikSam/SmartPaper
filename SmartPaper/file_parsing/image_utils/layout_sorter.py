...
class LayoutSorter:
    ...
    def __init__(self, threshold_left_right: float = 0.9, threshold_cross: float = 0.3):
        """
        初始化排序器
    ...
    def sort_layout(
        self, layout_result: Union[str, dict], image_path: str = None, page_width: float = None
    ) -> Dict:
        """
        对版面检测结果进行排序
    ...
    def _sort_elements(self, elements: List[Dict], page_width: float) -> List[Dict]:
        ...
        # 筛选有效元素
        valid_elements = [
            elem for elem in elements if "coordinate" in elem and len(elem["coordinate"]) == 4
        ]
        ...
        for elem in valid_elements:
            ...
            is_crossing = (
                left_ratio > self.threshold_cross and right_ratio > self.threshold_cross
            )  # 同时覆盖左右两侧

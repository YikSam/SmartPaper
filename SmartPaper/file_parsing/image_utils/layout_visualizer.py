...
class LayoutVisualizer:
    ...
    def _normalize_coordinate(self, coordinate: Any) -> Tuple[int, int, int, int]:
        ...
        try:
            ...
            if len(coordinate) == 4:
                ...
                len(coordinate) == 2
                and isinstance(coordinate[0], (list, tuple))
                and len(coordinate[0]) == 2
            ):
                # 格式: [[x1, y1], [x2, y2]]
                return (
                    int(coordinate[0][0]),
    ...
    def draw_boxes(
        self, image: np.ndarray, boxes: List[Dict], show_order: bool = True, show_label: bool = True
    ) -> np.ndarray:
        """绘制排序后的检测框
        ...
        for i, box in enumerate(boxes):
            ...
            if show_label:
                # 使用配置中的中文标签名(如果有)
                label_display = (
                    self.config.get_chinese_name(label)
                    if hasattr(self.config, "get_chinese_name")
                    else label
                )
            ...
            if text:
                ...
                # 绘制文本背景
                draw.rectangle(
                    [x1, y1 - text_height - 8, x1 + text_width + 8, y1], fill=color  # 增加内边距
                )

...
from SmartPaper.file_parsing.pdf_to_md_fitz_with_image import (
    extract_pdf_content as fitz_with_image_extract,
)
...
class FitzPDFConverter(PDFToMarkdownConverter):
    ...
    def convert(self, pdf_path: str, output_dir: str, **kwargs) -> str:
        ...
        return fitz_extract(
            pdf_path=pdf_path, output_dir=output_dir, strip_references=strip_references
        )
...
class PDFConverter:
    ...
    # 可用的转换器类型
    CONVERTER_TYPES = {"fitz": FitzPDFConverter, "fitz_with_image": FitzWithImagePDFConverter}
    ...
    def __init__(self, converter_type: str = "fitz"):
        ...
        if converter_type not in self.CONVERTER_TYPES:
            raise ValueError(
                f"不支持的转换器类型: {converter_type}。可用类型: {', '.join(self.CONVERTER_TYPES.keys())}"
            )
    ...
    def register_converter(cls, name: str, converter_class: type) -> None:
        ...
        if not issubclass(converter_class, PDFToMarkdownConverter):
            raise TypeError(
                f"转换器类必须继承PDFToMarkdownConverter，但获得了: {converter_class.__name__}"
            )
...
def convert_pdf_to_markdown(
    ...
    kwargs = {"strip_references": strip_references}
...
if __name__ == "__main__":
    ...
    markdown_basic = convert_pdf_to_markdown(
        pdf_path=pdf_path, output_dir=output_dir, converter_type="fitz", strip_references=True
    )

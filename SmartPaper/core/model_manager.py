...
class ModelManager:
    ...
    def get_llm(self, provider: Optional[str] = None, model: Optional[str] = None) -> Any:
        ...
        # 准备初始化参数
        init_params = {"api_key": api_key, "model": model}
...
def chat(
    ...
    return manager.chat(
        message=message, system_prompt=system_prompt, provider=provider, model=model, stream=stream
    )
...
def chat_with_history(
    ...
    return manager.chat_with_history(
        messages=messages, provider=provider, model=model, stream=stream
    )
...
def stream_with_history_callback(
    ...
    manager.stream_with_history_callback(
        messages=messages, callback=callback, provider=provider, model=model
    )
...
def list_available_models(
    provider: Optional[str] = None, config_path: Optional[str] = None
) -> List[str]:
    """
    便捷函数，列出指定提供商的可用模型

...
class OpenAILLM:
    ...
    def chat(
        self, message: str, system_prompt: str = "你是一个有用的AI助手。", stream: bool = False
    ...
    def _chat_stream(
        self, message: str, system_prompt: str = "你是一个有用的AI助手。"
    ) -> Iterator[str]:
        """
        进行流式单轮对话，返回迭代器
    ...
    def stream_with_history_callback(
        self, messages: List[Dict[str, str]], callback: Callable[[str], None]
    ) -> None:
        """
        使用回调函数处理带历史记录的流式响应
